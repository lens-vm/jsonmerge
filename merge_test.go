...
func mergePatch(doc, patch string) string {
	out, err := merge.MergePatch([]byte(doc), []byte(patch))
...
func TestMergePatchReturnsErrorOnBadJSON(t *testing.T) {
	_, err := merge.MergePatch([]byte(`[[[[`), []byte(`1`))

	if err == nil {
		t.Errorf("Did not return an error for bad json: %s", err)
	}

	_, err = merge.MergePatch([]byte(`1`), []byte(`[[[[`))
...
func TestMergePatchReturnsEmptyArrayOnEmptyArray(t *testing.T) {
	...
	res, err := merge.MergePatch([]byte(doc), []byte(pat))
...
func TestMergePatchFailRFCCases(t *testing.T) {
	...
	for _, c := range tests {
		...
		out, err := merge.MergePatch([]byte(doc), []byte(pat))
...
func TestMergeMergePatches(t *testing.T) {
	...
	for _, c := range cases {
		out, err := merge.MergeMergePatches([]byte(c.p1), []byte(c.p2))
